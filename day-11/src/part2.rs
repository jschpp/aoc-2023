...
impl Galaxy {
    ...
    pub fn expand(
        ...
        let factor = factor - 1;
        self.position.line_idx += factor
            * (empty_lines
                .iter()
                .filter(|x| x < &&self.position.line_idx)
                .count());
        self.position.column_idx += factor
            * (empty_columns
                .iter()
...
pub fn process(input: &str, factor: usize) -> String {
    ...
    let image: usize = image
        .into_iter()
        .map(|mut g| {
            g.expand(&empty_lines, &empty_columns, factor);
            g
        })
        .combinations(2)
...
#...#.....";
    ...
    fn test_input_1() {
        ...
        assert_eq!("9918828", process(input, 2));
    }

    #[test]
    fn test_input_2() {
        let input = include_str!("../input.txt");
        assert_eq!("692506533832", process(input, 1_000_000));
    }
}
